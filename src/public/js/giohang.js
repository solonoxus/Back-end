    ...
    s +=
      ...
      `</td>
				<td class="noPadding imgHide">
					<a target="_blank" href="../../views/Product/chitietsanpham.html?` +
      p.name.split(" ").join("-") +
      `" title="Xem chi tiết">
						` +
      p.name +
...
async function xoaSanPhamTrongGioHang(i) {
  if (window.confirm("Xác nhận hủy mua")) {
    ...
    capNhatMoiThu(); // Cập nhật giao diện
  }
}

async function thanhToan() {
  var c_user = await getCurrentUser(); // Lấy thông tin người dùng từ server
  if (c_user.off) {
