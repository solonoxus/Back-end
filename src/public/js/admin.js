...
function layThongTinSanPhamTuTable(id) {
  ...
  if (isNaN(price)) {
    // isNaN là hàm kiểm tra xem có phải là số không
...
async function suaSanPham(masp) {
  ...
  // Kiểm tra trùng mã sản phẩm và tên sản phẩm
  for (var p of list_products) {
...
function getValueOfTypeInTable_DonHang(tr, loai) {
  ...
  switch (loai) {
    ...
    case "trangthai":
      return td[6].innerHTML.toLowerCase(); // trả về trạng thái đơn hàng
  }
  return false;
}
...
function partition(arr, pivot, left, right, loai, func) {
  //patition là hàm chia mảng thành 2 phần để sắp xếp
...
function promoToStringValue(pr) {
  // promoToStringValue là hàm chuyển đổi giá trị khuyến mãi về dạng chuỗi tiếng việt
  switch (pr.name) {
    case "tragop":
      return "Góp " + pr.value + "%";
    case "giamgia":
