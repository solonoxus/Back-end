...
// ======================= Các Tab =========================
function addEventChangeTab() { // hàm thêm sự kiện click vào các tab 
    var sidebar = document.getElementsByClassName('sidebar')[0]; // biến sidebar là phần tử đầu tiên của class sidebar, getElementsByClassName sẽ lấy tất cả các phần tử có class là sidebar nhưng ta chỉ lấy phần tử đầu tiên nên ta lấy phần tử đầu tiên của class sidebar
    var list_a = sidebar.getElementsByTagName('a'); // biến list_a là tất cả các thẻ a trong sidebar
    for(var a of list_a) {  // vòng lặp for từng thẻ a trong list_a 
        if(!a.onclick) {
            a.addEventListener('click', function() {
                turnOff_Active();
                this.classList.add('active');
    ...
    fetch("/api/products", {
        ...
        .then((data) => {
          if (data.message) {
            alert(data.message);
            document.getElementById('khungThemSanPham').style.transform = 'scale(0)';
            addTableProducts(); // Cập nhật danh sách sản phẩm
          } else {
...
function autoMaSanPham(company) {
    ...
    .then((response) => response.json())
    .then((products) => {
      let index = products.filter((p) => p.company === company).length;
      document.getElementById("maspThem").value = company.substring(0, 3) + index;
    })
    .catch(() => alert("Lỗi kết nối tới server!"));
}
